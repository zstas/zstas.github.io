...
# Does have VPP its own IP/TCP stack?
Surely do. And there is possibility to force your legacy application to work with it!

We have VCL library in VPP. Let's see what functions it has.
...
All you need is export LD_PRELOAD with this library when running your application. Let's see how it works. I'm going to run 2 BGP daemons, one will be acting as peer for VPP. And the other will use VPP's host stack to connect to this peer.

# Let's build this setup!
Firt of all we need to create interface to outside world:
```
...
I chose BIRD to show this case, because it's only BGP daemon which doesn't fork. Because after fork LD_PRELOAD is break and I don't know how to fix it. Also I switched off several things in BIRD to make it working, there is the patch:
```
...
As you see above - it successfully bind on TCP/179 and also accepted the connection.
...
We can see this sessions in VPP:
```
...
# What's next
But unfortunately BIRD doesn't have an API to dump all the routing table (and install it through our VPP FIB Manager). So I'm still digging into it. Stay tuned!